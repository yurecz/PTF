...
CLASS lcl_aunit_listener DEFINITION FINAL CREATE PUBLIC.
  ...
  PUBLIC SECTION.
    ...
    CONSTANTS gc_kind_assertion_failed TYPE cl_ptf_wrapper=>ts_test_log-kind VALUE 'A'.
    CONSTANTS gc_kind_exception_raised TYPE cl_ptf_wrapper=>ts_test_log-kind VALUE 'X'.
    CONSTANTS gc_kind_runtime_error    TYPE cl_ptf_wrapper=>ts_test_log-kind VALUE 'R'.
    CONSTANTS gc_kind_warning          TYPE cl_ptf_wrapper=>ts_test_log-kind VALUE 'W'.

    DATA mt_failed    TYPE tt_alert READ-ONLY.
    DATA mt_fail_details TYPE STANDARD TABLE OF string READ-ONLY.
...
* ====================================
  METHOD if_aunit_listener~cx_failure.
    ...
    DATA(lt_all) = failure->get_complete_description( ).
    LOOP AT lt_all INTO DATA(ls_line) WHERE id(1) EQ 'A'.
      APPEND LINES OF ls_line-params TO mt_fail_details.
    ENDLOOP.
