...
CLASS ltc_build_operations DEFINITION FOR TESTING RISK LEVEL HARMLESS DURATION SHORT FINAL.
  PRIVATE SECTION.
    ...
    METHODS prodsa_cr_op_data FOR TESTING.
...
CLASS ltc_build_operations IMPLEMENTATION.
  METHOD class_setup.
  ...
  METHOD prodsa_cr_op_data.
    DATA lo_cut         TYPE REF TO cl_ptf_rap_operations.
    DATA lo_permissions TYPE REF TO cl_ptf_rap_permissions.
    ...
    lo_permissions = NEW #( lo_ptf_run ).

    lo_permissions->build_permissions(
      EXPORTING
...
*        ev_error            = lv_error
    ...
    cl_abap_unit_assert=>assert_not_initial( act = lt_operations[ entity_name = 'R_PRODUCTIONSUPPLYAREATP' sub_name = space ]-instances->* ).
    cl_abap_unit_assert=>assert_not_initial( act = lt_operations[ entity_name = 'R_PRODUCTIONSUPPLYAREATP' sub_name = '_PRODUCTIONSUPPLYAREATEXT' ]-instances->* ).

  ENDMETHOD.

ENDCLASS.

* Local Test Class for method handle_operations
CLASS ltc_handle_operations DEFINITION FOR TESTING RISK LEVEL HARMLESS DURATION SHORT FINAL.
  PRIVATE SECTION.
    CLASS-METHODS class_setup.
    CLASS-METHODS class_teardown.
    ...
    METHODS handle_operations_error FOR TESTING.

ENDCLASS.

CLASS ltc_handle_operations IMPLEMENTATION.
  METHOD class_setup.
    "Test double would be created for the CDS R_ProductionSupplyAreaTP. Executes once per test class.
  ...
  METHOD handle_operations_error.
    DATA lo_cut         TYPE REF TO cl_ptf_rap_operations.
    DATA lt_step_data   TYPE cl_ptf_util=>gt_ptf_step_tab.
    DATA ls_step_data   TYPE cl_ptf_util=>gt_ptf_step.
    ...
    APPEND VALUE #( root_name = 'R_PRODUCTIONSUPPLYAREATP' entity_name = 'R_PRODUCTIONSUPPLYAREATP' ) TO lt_failed ASSIGNING FIELD-SYMBOL(<fs_failed>).
    ...
    "when
    lo_cut->handle_operations_error(
      EXPORTING
        it_failed           = lt_failed
        it_mapped           = lt_mapped
