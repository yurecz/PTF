*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_V_PTF_TAG_MAP
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_V_PTF_TAG_MAP       .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_V_PTF_TAG_MAP'
         EXPORTING
              FCODE          = READ
...
EXTRACT = V_PTF_TAG_MAP_EXTRACT
TOTAL = V_PTF_TAG_MAP_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_V_PTF_TAG_MAP'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_V_PTF_TAG_MAP-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = V_PTF_TAG_MAP_EXTRACT
TOTAL = V_PTF_TAG_MAP_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_V_PTF_TAG_MAP-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_V_PTF_TAG_MAP'
...
TOTAL = V_PTF_TAG_MAP_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_V_PTF_TAG_MAP-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_V_PTF_TAG_MAP-UPD_FLAG NE SPACE.
